...
```sh
tools/gates.sh              # verify, cleanroom, check-docs, check-tooling -- true exit status
...
gmake check-tooling # every tools/test_*.py and tests/test_*.py, discovered not listed
```
...
| `gmake check-tooling` | every discovered `tools/test_*.py`/`tests/test_*.py` still passes. Discovered, not hand-listed (`tools/run_tool_tests.py`), so a new test file is never silently skipped; runs against fixtures, so it needs no build | no |
