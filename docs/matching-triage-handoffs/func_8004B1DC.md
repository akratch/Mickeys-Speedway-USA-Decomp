...
- historical pre-packet configured baseline and retained candidate, superseded on 2026-09-24: 2,192 candidate bytes / 548 words against 2,224 target bytes / 556 words; size delta was -32 bytes, with 466 raw and 465 relocation-masked differing words; both frames were `0x80`. Those -32 and 465 figures are not the current baseline. The current baseline is the header above: size delta 0, 452 masked words, frame 0x80.
...
- stopping evidence, historical pre-packet note superseded on 2026-09-24: the claim of zero material C attempts is withdrawn. The kept postincrement spelling recorded in the remeasure above, Gfx *packet = dList++, is the current body and a later reader should not skip it. Baseline compilation and donor-history inspection from that earlier pass are not those postincrement attempts. The early-exhaustion claim and the prior ten-attempt plateau do not forbid that kept spelling.
