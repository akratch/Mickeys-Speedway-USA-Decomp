...
`overlay 14 +0x1830..+0x1B54` (`func_overlay_014_F0001830_1871108`) — **804 exact C bytes / 201 words**, frame `0x90`, 14 of 14 relocation identities (2026-09-23, lane A2-ov). Track B (lane B-ovsmall) closed size and frame by declaring the font-width call int-returning; the last five words were case 7's schedule, an as1 `lineno` tie (L59) between the stream increment and the four colour loads. The increment needs the later source line, so the draw block sits before the `case 7:` label and the case body is the increment plus a jump back; uopt merges the blocks. The compiler's jump-table pool is externalized onto the retained table at data_rodata `+0x174` in overlay 14's metadata-only form. `gmake verify` and `promotion-proof` pass from the C; the route is the function's shard.

`overlay 35 +0x01E0..+0x0770` (`func_overlay_035_F00001E0_1881EC0`) — **1,424 exact C bytes / 356 words**, frame `0x40`, 63 of 63 relocation identities (2026-09-23, lane B3-rl35). Track B took it from size delta -8 to 0 and then to 0 words, unforced. Size: every model access spelled `D_o35_current_model->...` as in DKR's `generate_track`, so uopt reuses each loop condition's reload (the second loop's preheader then starts from constants), the two copy loops written `while (n--)` (the target's dead count copy), and the colour walkers read back from the just-stored `colorData` fields. Naming: the heap test reads `D_o35_current_model`, the model-table scan walks `D_o35_model_table` itself (its address web then outranks `0x9F000` for `s3`), the segment count takes its own local, and the destination is `temp = model; temp += 0x9F000 - size`. One unused pointer holds the frame (`docs/cleanup-queue.md`). `gmake verify` and `promotion-proof` pass from the C; the route is the function's shard.

`overlay 8 +0x1000..+0x1294` (`func_overlay_008_F0001000_185ED58`) — **660 exact C bytes / 165 words**, frame `0x18`, 18 of 18 relocation identities (2026-09-23, lane A2-ov). The last seven words were the rollover clamp: the shipped `b` to the next word crosses an else arm that ugen emits and as1 empties, and that else belongs to the rollover-control test (`count = 120`), not to the clamp. With the default there the sample is assigned straight to the count and as1 folds the only copy; an inner-else copy kept count and sample in interfering webs. `gmake verify` and `promotion-proof` pass from the C; the route is the function's shard.
