...
| `func_80037414` | 580 bytes under `-O2 -mips2 -32`; VI transition body, all 145 instruction words and its relocation surface exact (matched 2026-09-23: argument-less `func_800371BC` call, address-ordered tail stores, CSE'd first frame count, and a TrapDanglingJump call naming only `arg0`: a1/a2 already hold `var_a1`/`var_a2` at the jump, and naming `var_a1` lets uopt copy-propagate the expression temp into it). |
