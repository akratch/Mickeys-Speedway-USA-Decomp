...
| `func_800371BC` | 600 bytes under `-O2 -mips2 -32`; backdrop grid initialisation, all 150 instruction words and its relocation surface exact (matched 2026-09-23: `(void)` signature, s32 row value, index loop over the buffer table so strength reduction makes the cursor, store-then-reload of the allocation, `for` row loop, u8 colour bytes). |
| `func_80037414` | 580 bytes under `-O2 -mips2 -32`; VI transition body, all 145 instruction words and its relocation surface exact (matched 2026-09-23: argument-less `func_800371BC` call, address-ordered tail stores, CSE'd first frame count, and `var_a1 \| 0` as the TrapDanglingJump argument so uopt does not copy-propagate the expression temp into it). |
...
| `func_80012234` | `0x12E34` | 0x340 | `-O2 -mips2 -32 -Wab,-r4300_mul` | Mickey m2c reconstruction of the plane/cylinder interval query; JFG's assembly-only `trackCylinderIntersect` is role context only, no external body is adapted. Matched 2026-09-23 (Track B): left-associated sums and textbook cross-product operand order close the +12 size delta, normalLength declared first takes the top home of the 0x60 frame, and the dot product assigned straight into absoluteDot (no scalar carrier) gives it $f2 ahead of the stack radius; 208/208 instruction words and all 3 relocation records exact, linked ROM exact |
| `func_80012658` | `0x13258` | 0x2C4 | `-O2 -mips2 -32 -Wab,-r4300_mul` | Mickey reconstruction of the collision-edge record builder from its own node, face and output-record accesses; no external body is adapted. Matched 2026-09-23 (Track B): face and vertex-base cursors formed once per node from index locals, the corner flag re-read through the node, the output record addressed as count + base, and two unreferenced locals ahead of the node pointer for its +0x34 home in the 0x40 frame; 177/177 instruction words and all 18 relocation records exact, linked ROM exact |
...
| `func_800133FC` | `0x13FFC` | 0x180 | `-O2 -mips2 -32 -Wab,-r4300_mul` | Mickey reconstruction of the three-point plane helper; the reference scan found no credible donor. Matched 2026-09-23 (Track B): one local per coordinate, delta and normal component replacing the m2c carriers, deltas declared before the normal with three unreferenced locals between (the 0xA0 frame's home layout), and deltas computed grouped by axis; 96/96 instruction words and the `sqrtf` relocation exact, linked ROM exact |
